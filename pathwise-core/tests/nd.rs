...
fn heston_variance_nonnegative() {
    ...
    for path in 0..500 {
        for step in 0..=200 {
            ...
            assert!(
                v >= -0.05 || v.is_nan(),
                "variance negative: {} at path={} step={}",
                v,
                path,
                step
            );
...
fn corr_ou_covariance() {
    ...
    let cov00: f64 = (0..n_paths)
        .map(|i| (out[[i, last_step, 0]] - mean0).powi(2))
        .sum::<f64>()
        / (n_paths - 1) as f64;
    let cov11: f64 = (0..n_paths)
        .map(|i| (out[[i, last_step, 1]] - mean1).powi(2))
        .sum::<f64>()
        / (n_paths - 1) as f64;
    let cov01: f64 = (0..n_paths)
        .map(|i| (out[[i, last_step, 0]] - mean0) * (out[[i, last_step, 1]] - mean1))
        .sum::<f64>()
        / (n_paths - 1) as f64;
    ...
    println!(
        "CorrOU cov01={:.4} (expected {:.4})",
        cov01, expected_offdiag
    );
    assert!(
        (cov00 - expected_diag).abs() / expected_diag < 0.05,
        "cov00 {:.4} vs {:.4}",
        cov00,
        expected_diag
    );
    assert!(
        (cov11 - expected_diag).abs() / expected_diag < 0.05,
        "cov11 {:.4} vs {:.4}",
        cov11,
        expected_diag
    );
    assert!(
        (cov01 - expected_offdiag).abs() / expected_offdiag < 0.10,
        "cov01 {:.4} vs {:.4}",
        cov01,
        expected_offdiag
    );
...
fn milstein_nd_stronger_than_euler_nd_on_gbm_like() {
    ...
    assert!(
        finite_mil > n_paths * n_steps / 2,
        "too many NaN in MilsteinNd"
    );
    assert!(
        finite_euler > n_paths * n_steps / 2,
        "too many NaN in EulerNd"
    );
}
